...
int main(int argc, char **argv)
{
	...
	int cmd_counter = 1                       ; /*** For (P5.2) ***/
	char history_buffer[9][SHELL_BUFFER_SIZE];  /*** For (P5.3) ***/
	int sub_shell_depth = 0;                    /*** For (P5.6) ***/
	...
	/* The Shell runs in an infinite loop, processing input. */
		...
		/*** Check if we're executing a previous command. (P5.3) ***/
		if (buffer[0] == '!') {
			int prev_cmd = buffer[1] - '0';
			if ((1 <= prev_cmd) && (prev_cmd <= 9) && (prev_cmd < cmd_counter)) {
		...
		/*** Maintain command counter & history (P5.3) ***/
		if (buffer[0] != '\n') {
			if (cmd_counter <= 9) {
				strncpy(history_buffer[cmd_counter-1], buffer, SHELL_BUFFER_SIZE);
			}
			cmd_counter++;
		}
		...
		/* Execute Commands */
			...
			if (pid_from_fork == 0) {
				/*** Added support for subshell (P5.6) ***/
				if (!strcmp(exec_argv[0], "sub")) {
					/*** Reinitialize the subshell (P5.6) ***/
					cmd_counter = 1;
					shell_pid   = getpid();
					sub_shell_depth++;
