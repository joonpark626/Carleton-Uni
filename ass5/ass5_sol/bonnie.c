...
/* main entry point for ASSIGNMENT5 */
int bonnie_main(int argc, char **argv)
{
	...
	if (next == 0) {	/* first time */
	    ...
	    first_start =
		(first_start <
		 seeker_report[StartTime]) ? first_start :
		seeker_report[StartTime];
	    last_stop =
		(last_stop >
		 seeker_report[EndTime]) ? last_stop :
		seeker_report[EndTime];
	}			/* not first time */
	if (wait(&child) == -1)
	    io_error("wait");
	fprintf(stderr, "done...");
...
static void report(char *machine, off_t size)
{
    ...
    printf("%%CPU  /sec %%CPU\n");
    printf("%-8.8s %4d ", machine, (int) (size / (1024 * 1024)));
    printf("%5d %4.1f %5d %4.1f %5d %4.1f ", 
	     (int) (((double) size) /
		    (delta[(int) Putc][Elapsed] * 1024.0)),
	     delta[(int) Putc][CPU] / delta[(int) Putc][Elapsed] * 100.0,
	     (int) (((double) size) /
		     (delta[(int) FastWrite][Elapsed] * 1024.0)),
	     delta[(int) FastWrite][CPU] /
	     delta[(int) FastWrite][Elapsed] * 100.0,
	     (int) (((double) size) /
...
static void usage()
{
    fprintf(stderr, "usage: Bonnie [-s size-in-Mb] [-m machine-label]\n");
    exit(1);
} 
...
static void file_read_rewrite_block()
{
    ...
    size_t offset = 0;
    /* offset is new: we need an explicit variable to hold where we are in the file */ 
	
    if ((words = file_read(name, offset, buf, Chunk)) < 0)
	...
	if (bufindex == Chunk / IntSize)
	    bufindex = 0;
	buf[bufindex++]++;
	if (file_write(name, offset, buf, words) < 0)
	    io_error("re write(2)");
	offset += words;
	
        /* Be careful of the order: otherwise the file_write will keep extending the file and never finish */ 
        if ((words = file_read(name, offset, buf, Chunk)) < 0)
	    io_error("rewrite read (3)");
    }
